...
export function useDevisStore() {
  ...
  const addDevis = useCallback(async (data: { clientId: string; lignes: { description: string; quantite: number; prixUnitaire: number }[] }) => {
  ...
  const createOperationFromDevis = useCallback(async (devis: Devis) => {
    ...
    const description = devis.lignes.map((l) => l.description).join(", ");
